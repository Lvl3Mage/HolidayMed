<script setup>
	...
	const advantagesGroup = reactive(useValidationGroup());
	...
	defineExpose({
		isValid: () => inputGroup.isValid() && priceGroup.isValid() && advantagesGroup.isValid(),
		GetTitle
	});
</script>
<template>
		...
		<div role="tabpanel" class="tab-content">
			...
			<!-- <div class="collapse collapse-arrow bg-base-200 my-5" :class="{'ring-1 ring-error': !advantagesGroup.isValid()}">
				<input type="checkbox" /> 
				<div class="collapse-title text-xl font-medium">
					Ventajas
				</div>
				...
				<div class="collapse-content"> 
					<div class="">
						<div class="group/seasonalPrice" v-for="(seasonalPrice,id) in getAcf().prices.season_prices" :key="id">
							<div class="flex">
								<div class="join join-horizontal sm:join-vertical w-0 grow">
									<TextInput :ref="el => advantagesGroup.elements[`advantages${id}text`] = el" v-model="seasonalPrice.base_price" placeholder="100" :validate="formValueValidation.notEmpty" type='text' class="join-item grow min-w-0 basis-0 sm:basis-auto">
										<template v-slot:before><span class="font-bold">Texto:</span></template>
									</TextInput>
								</div>
